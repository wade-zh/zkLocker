...
/***
 * zkClient操作类
 * 本操作类来自https://blog.csdn.net/sunfeizhi/article/details/51926396
 */
public class BaseZkLocker {
    /**
     * 第三方客户端
     */
     ...
     * 锁前缀名称
     */
    private final String nodePrefixName;

    public BaseZkLocker(ZkClient client, String path, String nodePrefixName) {
        this.client = client;
        this.basePath = path;
        this.path = path.concat("/").concat(nodePrefixName);
        this.nodePrefixName = nodePrefixName;
    }
            ...
            Collections.sort(
                    ...
                    new Comparator<String>() {
                        public int compare(String lhs, String rhs) {
                            return getLockNodeNumber(lhs, nodePrefixName).compareTo(getLockNodeNumber(rhs, nodePrefixName));
                        }
                    }
            );
            return children;
    ...
    protected String attemptLock(long time) throws Exception {
        final long startMillis = System.currentTimeMillis();
        final Long millisToWait = TimeUnit.MILLISECONDS.toMillis(time);
